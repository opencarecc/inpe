...
boolean lcd_draw_font(void* user_data)
{
    ...
    // vm_graphic_draw_solid_rectangle(&g_frame, data->ulX, data->ulY, strNum * (data->ulSize)/1.7, data->ulSize);
    ...
    VMCHAR c;
  	while (data->str[i])
  	{
    	...
    	if (isupper(c))
	    {
	    	vm_graphic_draw_solid_rectangle(&g_frame, data->ulX, data->ulY, strNum * (data->ulSize)/1.5, data->ulSize);
	    } else {
	    	vm_graphic_draw_solid_rectangle(&g_frame, data->ulX, data->ulY, strNum * (data->ulSize)/1.7, data->ulSize);
	    }
    	putchar (c);
...
boolean lcd_draw_number(void* user_data)
{
    ...
    vm_graphic_draw_solid_rectangle(&g_frame, data->ulX, data->ulY, j * (data->ulSize)/1.7, data->ulSize);
    if (j==0)
    {
    	vm_graphic_draw_solid_rectangle(&g_frame, data->ulX, data->ulY, (data->ulSize)/1.7, data->ulSize);
    }   
