...
void displayDate(){
  if(checkGPS()==1){
    ...
    Lcd.draw_font(10, 7, bufferDateTime, 0xffffff, 0x000000, 20);  
  } else {
    Lcd.draw_font(10, 7, "(Date Error)", 0xffffff, 0x000000, 20);
  }
}
void displayTime(){
  if(checkGPS()==1){
    sprintf(bufferDateTime, " %d : %d : %d ", (utc_date_time[3])+timeZone, utc_date_time[4], utc_date_time[5]);
    Lcd.draw_font(65, 65, bufferDateTime, 0, 0xffffff, 20);
  } else {
    Lcd.draw_font(65, 65, "(Time Error)", 0, 0xffffff, 20);
  }
}
...
void displaySensorHub(){
  Lcd.draw_font(10, 200, "Acc:", 0, 0xffffff, 20);
  if(checkSensorHub()){
    // if SensorHub is connected display sensorHubStatus in white
    Lcd.draw_font(10, 220, sensorHubStatus, 0, 0xffffff, 20);
  } else {
    // if SensorHub is connected display sensorHubStatus in red
    Lcd.draw_font(10, 220, sensorHubStatus, 0, 0xff0000, 20);
  }
}
...
void displayGPS(){
  Lcd.draw_font((xFinal/2)-30, 200, "GPS:", 0, 0xffffff, 20);
  if (LGPS.check_online()){
    // if GPS is connected display GPSState in white
    Lcd.draw_font((xFinal/2)-30, 220, GPSState, 0, 0xffffff, 20);
    if (LGPS.get_status()==65) {
      // if GPS status is A display GPSStatus in white
      Lcd.draw_font((xFinal/2)+8, 220, GPSStatus, 0, 0xffffff, 20);
    } else {
      // if GPS status is V or error display GPSStatus in red
      Lcd.draw_font((xFinal/2)+8, 220, GPSStatus, 0, 0xff0000, 20);
    }
  } else {
    // if GPS is connected display GPSState in red
    Lcd.draw_font((xFinal/2)-30, 220, GPSState, 0, 0xff0000, 20);
  }
}
...
void displaySIM(){
  Lcd.draw_font(190, 200, "SIM:", 0, 0xffffff, 20);
  if(LCheckSIM.isCheck()){
    Lcd.draw_font(190, 220, SIMStatus, 0, 0xffffff, 20);
  } else {
    Lcd.draw_font(190, 220, SIMStatus, 0, 0xff0000, 20);
  }
}
...
// flush content by creating a new background
  ...
  while(systemFlag==0 && flag==0){
    ...
    Lcd.draw_font(85, 105, "In Pe' is", 0, 0xffffff, 20);
    if(checkSystem()==1){
      // font color green
      draw_rect(50,130,150,40,0x000000);
      Lcd.draw_font(89, 130, systemCheck, 0, 0x00ff00, 20);
    } else {
      // font color red
      draw_rect(50,130,150,40,0x000000);
      Lcd.draw_font(89, 130, systemCheck, 0, 0xff0000, 20);
      Lcd.draw_font(50, 150, "Check System >", 0, 0xff0000, 20);
    }
